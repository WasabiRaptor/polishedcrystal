...
.quick_powder
	...
	jr z, .got_species_and_group
	ld a, [wEnemyMonSpecies]
	ld b, a
	ld a, [wEnemyMonGroup]
.got_species_and_group
	cppoke DITTO, .done
	ld a, $21
.apply_item_mod
	call ApplyDamageMod
...
FinalPkmnMusicAndAnimation:
	...
	ret nc
	; ...and this is their last Pok�mon...
	farcall CheckAnyOtherAliveEnemyMons
	ret nz
	; ...then hide the Pok�mon...
	call EmptyBattleTextBox
	ld c, 20
	call DelayFrames
	call SlideEnemyPicOut
	; ...play the final Pok�mon music...
...
; runs both pok�mon's activation abilities (Intimidate, etc.).
; The faster Pok�mon activates abilities first. This mostly
...
; Trace will, on failure, copy a later switched in Pok�mon's
...
; Called when a Pok�mon with Mold Breaker uses Roar/Whirlwind.
...
; c: loop iterator
; d: bit 0 is set on pok�rus, bit 1 on macho brace
...
.store_result
	...
	farcall AddLastBattleToLinkRecord
